...
def setup_experiment(args):
    # BUG: SyntaxError: name 'prompt_template' is used prior to global declaration
...
def main():
    ...
    if args.resume:
        ...
        if os.path.exists(partial_result_base_dir):
            ...
            # print("partial_files", partial_files)
            if partial_files:
                # 提取所有 partial 文件中的索引
                indices = []
    ...
    # 从 resume_start 开始处理（假设数据集支持切片操作）
    for idx, item in enumerate(tqdm(dataset[resume_start:], desc="Processing items"), start=resume_start + 1):
        ...
        # 每处理 save_interval 个数据，保存一次中间结果
        save_interval = 100
